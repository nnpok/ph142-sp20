...
source("setup/lab01.RAGS.R")
```

### Instructions 
* Due date: Friday, Jan 24 at 10:00pm.
...
**IMPORTANT NOTE:** Due to a sudden unforeseen issue with UC Berkeley's Datahub, the submission system is not currently working as of 1/23/2020. We expect the issue to be fixed by 1/25/2020. For this assignment, there is no need to submit.
...
Starting this semester, we are introducing an autograder for this class. Basically, we've written a series of test cases and checkpoints that test the correctness of your code and allows you to check your work and receive automated real-time feedback on your answers. 

At the beginning of all assignments, you'll see this setup chunk below, which loads the autograder. Make sure to run this chunk first, always. 
...
This sets up the autograder environment. If you look in the "Environment" tab, you should see a bunch of objects appear, including `max_scores`, `num_tests`, etc. Don't worry about these objects and don't delete them, as they are used by the autograder.

Now, let's try solving a problem. In your assignments, you be given a prompt and be asked to code the answer and assign it to an object. Do not change the name of this object. **For this problem, set `p1` to a mathematical expression that evaluates to 5.** You'll often get starter code that will look like this:
...
As you work on the problem, replace the text `"<<<<YOUR CODE HERE>>>>"` with your solution to the problem. Running `check_problem1()` will then tell you if you're right or wrong, and sometimes provide helpful hints. Let's try.

```{r five-try}
# Replace the text below with your answer. (try p1 <- 2 + 3)
p1 <- "<<<<YOUR CODE HERE>>>>"

check_problem1()
```

When you enter the correct answer, `check_problem1()` should tell you all checkpoints passed, and all tests pass for that problem. Now... what if we don't know how to follow directions? Let's see what happens when you enter in a wrong answer. Run the following two chunks in order and observe their output.
...
Now, at the end of the assignment, there will be a chunk for you to check your progress/final score before submitting. Run the chunk below to see what that looks like. Since we got p1 correct, and p2 and p3 incorrect, we should expect to see that reflected. You'll also see the possible number of points per question. Unfortunately, our tech in this course isn't good enough (yet!) to autograde free-response questions, so you'll see in the score report that these are **NOT YET GRADED** and will be graded later by your GSIs. 
...
However, **homework** is designed for you to practice application, so we will only be releasing a subset of the test cases to check correctness. This means that the tests will only check that you're in the right ballpark. For example, if the answer to a problem is 42, the test might check that your answer is in the range 0-100, and not something outrageous like 142000 or "apple". Your grade on the assignment will be based on more rigorous test cases that we run on your code after you submit. Since this is much more challenging, homework is graded **as a percentage of answers correct, over all possible points.** We hope this design fosters strong computational independence!
...
cd; cd ph142-sp20/lab/lab01; python3 turn_in.py
