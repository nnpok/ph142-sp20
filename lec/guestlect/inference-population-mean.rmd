...
- In contrast, $s/\sqrt{n}$ is the estimate for the standard error of the mean,
$\bar{x}$. $s/\sqrt{n}$ estimates the variation between *means*.
...
ggplot(data = data.frame(x = c(-3, 3)), aes(x)) +
  ...
  scale_color_manual(values = c("black", "#1b9e77", "#d95f02"), name = "Distribution") +
...
be ask to compare the p-value to a pre-defined significance level, $\alpha$. Often 
$\alpha = 0.05$
...
group of children is abnormally high.
...
ggplot(pwv_dat, aes(x = pwv)) +
  ...
  geom_text(aes(x=5.5, y = 3.5, label = "H0\n(null\nhypothesis)"), check_overlap = T, col = "blue") +
...
except when outliers or a strong skew are present.
